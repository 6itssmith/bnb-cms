...
export default function BookingDetailDrawer({
  ...
  const canCancel = canManage && (booking.status === "pending_payment" || booking.status === "confirmed");
  const canRefund = canManage && booking.status === "confirmed" && booking.payment_method;
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md h-full bg-white dark:bg-ink overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between px-5 h-16 border-b border-earth/10 dark:border-cream/10">
          <h2 className="font-bold text-earth-dark dark:text-cream">{booking.guest_name}</h2>
        ...
        <div className="p-5 space-y-6">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Check-in</p>
              <p>{format(new Date(`${booking.check_in}T00:00:00`), "d MMM yyyy")}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Check-out</p>
              <p>{format(new Date(`${booking.check_out}T00:00:00`), "d MMM yyyy")}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Guests</p>
              <p>{booking.guests}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Status</p>
              <p className="capitalize">{booking.status.replace("_", " ")}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Email</p>
              <p className="truncate">{booking.guest_email}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Phone</p>
              <p>{booking.guest_phone ?? "—"}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Total</p>
              <p>
                {booking.currency} {Number(booking.total_amount).toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-xs font-bold text-ink/50 dark:text-cream/50">Payment method</p>
          ...
          <div>
            ...
            />
            {canManage && (
              <button onClick={saveNotes} disabled={saving} className="btn-secondary text-xs mt-2">
                {saving && <LoaderCircle className="w-3 h-3 animate-spin" aria-hidden="true" />}
                Save notes
              </button>
            )}
          </div>
          ...
          {canManage && (canCancel || canRefund) && (
            <div className="border-t border-earth/10 dark:border-cream/10 pt-5 space-y-3">
              ...
              {canRefund && confirmRefund && (
                <div className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/30 p-3 space-y-2">
                  <p className="text-xs text-red-700 flex items-center gap-1.5">
                    ...
                    This issues a real refund via {booking.payment_method} and marks the booking
                    cancelled. This can't be undone from here.
                  </p>
                  <div className="flex gap-2">
                    <button onClick={refundBooking} disabled={busy !== null} className="btn-primary flex-1 text-xs">
                      {busy === "refund" && <LoaderCircle className="w-3 h-3 animate-spin" aria-hidden="true" />}
                      Confirm refund
                    </button>
                    <button onClick={() => setConfirmRefund(false)} className="btn-secondary flex-1 text-xs">
                      Back
                    </button>
                  </div>
                </div>
              ...
              {canCancel && !canRefund && (
                <button onClick={cancelBooking} disabled={busy !== null} className="btn-secondary w-full text-red-600 border-red-200">
                  {busy === "cancel" && <LoaderCircle className="w-3 h-3 animate-spin" aria-hidden="true" />}
                  Cancel booking
                </button>
              )}
            </div>
